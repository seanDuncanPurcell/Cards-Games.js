...
const Interface = (function () {
  ...
  const _newCard = (suit) => {
    ...
    while(i < imgArr[0].length){
      ...
      img.src = `http://${location.host}/projects/blackJack/img/${imgArr[0][i]}`;
  ...
  const _endRound = () => {
    ...
    for (let i = 0; i < playerHands.length; i++){
      ...
      if (result == "Win"){
        ...
        winnings += `Your Hand 0${i + 1}: Wins $${playerBets[i]}.00 `;
      }
      else if (result == "Push"){
        _players[1].wallet += (playerBets[i]);
        winnings += `Your Hand 0${i + 1}: Pushed `;
      }
      else if (result == "BlackJack"){
        let bet = (playerBets[i] * 2.5);
        _players[1].wallet += bet;
        winnings += `Your Hand 0${i + 1}: Wins $${playerBets[i] * 1.5} `;
      }
      else if (result == "Lose"){
        let bet = (playerBets[i]);
        winnings += `Your Hand 0${i + 1}: Loses $${bet}.00 `;
      }
      playerBets[i] = 0;
    ...
    winnings += 'Click Here For Another Hand.'
